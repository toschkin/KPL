...
#define GVN_COLUMNRESIZE        WM_USER+55// Novodranov
...
public:
    ...
    BOOL SetRowHeight(int row, int height,/*Novodranov ->*/BOOL bResetScroll = TRUE);
    int  GetColumnWidth(int nCol) const;
    BOOL SetColumnWidth(int col, int width,/*Novodranov ->*/BOOL bResetScroll = TRUE);
...
public:
	...
	//Novodranov
	BOOL   SetRowBkColour(int nRow, COLORREF cr = CLR_DEFAULT,BOOL bPaintFixed=FALSE);    
    BOOL   SetRowFgColour(int nRow, COLORREF cr = CLR_DEFAULT,BOOL bPaintFixed=FALSE);    
	BOOL   SetColumnBkColour(int nColumn, COLORREF cr = CLR_DEFAULT,BOOL bPaintFixed=FALSE);    
    BOOL   SetColumnFgColour(int nColumn, COLORREF cr = CLR_DEFAULT,BOOL bPaintFixed=FALSE);    
...
public:
    int  InsertColumn(LPCTSTR strHeading, UINT nFormat = DT_CENTER|DT_VCENTER|DT_SINGLELINE,
                      int nColumn = -1);
    int  InsertRow(LPCTSTR strHeading, int nRow = -1,/*Novodranov ->*/BOOL bResetScroll=TRUE);
    BOOL DeleteColumn(int nColumn);
    BOOL DeleteRow(int nRow,/*Novodranov ->*/BOOL bResetScroll=TRUE);
...
protected:
	...
	// Victor sdelal public
public:
    void ResetSelectedRange();
public:// Novodranov sdelal public
...
*/
// Novodranov Прикручиваю поиск в грид
public:
	CFindReplaceDialog* m_pFindDialog;
	static const UINT m_FindDialogMessage;
	void OnFind(void);
//печать колонтитулов
	void SetPrintLeftHeader(CString strLeftHeader){m_strCenterHeader=strLeftHeader;m_bMoveHeaderToLeft = TRUE;}
	void SetPrintCenterHeader(CString strCenterHeader){m_strCenterHeader=strCenterHeader;m_bMoveHeaderToLeft = FALSE;}
	void SetPrintRightHeader(CString strRightHeader){m_strRightHeader=strRightHeader;}
	void SetPrintLeftFooter(CString strLeftFooter){m_strLeftFooter=strLeftFooter;}
	void SetPrintRightFooter(CString strRightFooter){m_strRightFooter=strRightFooter;}
	void MovePrintHeaderToLeft(void){m_bMoveHeaderToLeft = TRUE;};
	void MovePrintHeaderToCenter(void){m_bMoveHeaderToLeft = FALSE;};
	//void SetPrintHeaderFont(CFont* font){m_PrinterFont = &font;}
	void SetPrintHeaderFontParameters(LONG lWeight,LONG lWidth,LONG lHeight){m_lPrintFontWeight=lWeight;m_lPrintFontWidth=lWidth;m_lPrintFontHeight=lHeight;}
	//CFont GetPrintHeaderFont(void){return m_PrinterFont;}
	BOOL m_bMoveHeaderToLeft;
	LONG m_lPrintFontWeight;
	LONG m_lPrintFontWidth;
	LONG m_lPrintFontHeight;	

	CString	m_strCenterHeader;
	CString	m_strRightHeader;
...
protected:
	...
	afx_msg LRESULT OnFindDialogMessage(WPARAM wParam, LPARAM lParam);//Novodranov
