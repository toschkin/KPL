...
{
    const int nMaxWidth = 500;  // don't let the box be bigger than this
...
int CGridCellCombo::GetCurSel(void)//Novodranov
{	
	CString strTempText = GetText();
...
void CGridCellCombo::SetCurSel(int index)//Novodranov
{	
...
void  CGridCellCombo::SetItemDataArray(const  CArray<DWORD,DWORD>& arrItemData)//Novodranov
{
	m_arrItemData.Copy(arrItemData);
}
DWORD CGridCellCombo::GetItemData(int index)//Novodranov
{
	if((index >= 0)&&(index < m_arrItemData.GetSize()))
	{
		return m_arrItemData[index];
	}
	return -1;
}