...
//Novodranov
const UINT CGridCtrl::m_FindDialogMessage = RegisterWindowMessage(FINDMSGSTRING);
...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	//Novodranov
	m_bMoveHeaderToLeft = FALSE;
	m_pFindDialog = NULL;
	m_lPrintFontWeight=-1;
	m_lPrintFontWidth=-1;
	m_lPrintFontHeight=-1;	
...
//}}AFX_MSG_MAP
	...
	ON_REGISTERED_MESSAGE(m_FindDialogMessage, OnFindDialogMessage)//Novodranov
END_MESSAGE_MAP()
...
{
	if(IsSHIFTpressed() && IsCTRLpressed() && (nChar == 3))
		CopyToClipboard();
...
void CGridCtrl::OnHScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
	...
	SendMessageToParent(-1,-1,LVN_ENDSCROLL);
}
...
void CGridCtrl::ResetScrollBars()
{
    ...
    rect.top += GetFixedRowHeight();
	 ...
	 //Removed by Novodranov Anton to fix scrollbars on dialog problem
    /*if (rect.left >= rect.right || rect.top >= rect.bottom)
    {
...
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */,/*Novodranov ->*/BOOL bResetScroll)
{
...
BOOL CGridCtrl::DeleteRow(int nRow,/*Novodranov ->*/BOOL bResetScroll)
{
    if (nRow < 0 || nRow >= GetRowCount())
        return FALSE;
...
//Novodranov
BOOL CGridCtrl::SetRowBkColour(int nRow, COLORREF cr,BOOL bPaintFixed)
{
	BOOL bRet= TRUE;
	for(int col = 0; col <  GetColumnCount(); col++)
	{						
		if((!bPaintFixed)&&(GetItemState(nRow,col) & GVIS_FIXED))
			continue;
		bRet = SetItemBkColour(nRow,col,cr);		
		if(!bRet)
			break;
	}
	return bRet;
}
BOOL CGridCtrl::SetRowFgColour(int nRow, COLORREF cr,BOOL bPaintFixed)
{
	BOOL bRet= TRUE;
	for(int col = 0; col <  GetColumnCount(); col++)
	{						
		if((!bPaintFixed)&&(GetItemState(nRow,col) & GVIS_FIXED))
			continue;
		bRet = SetItemFgColour(nRow,col,cr);		
		if(!bRet)
			break;
	}
	return bRet;
}
BOOL CGridCtrl::SetColumnBkColour(int nColumn, COLORREF cr,BOOL bPaintFixed)
{
	BOOL bRet= TRUE;
	for(int row = 0; row <  GetColumnCount(); row++)
	{						
		if((!bPaintFixed)&&(GetItemState(row,nColumn) & GVIS_FIXED))
			continue;
		bRet = SetItemBkColour(row,nColumn,cr);		
		if(!bRet)
			break;
	}
	return bRet;
}
BOOL CGridCtrl::SetColumnFgColour(int nColumn, COLORREF cr,BOOL bPaintFixed)
{
	BOOL bRet= TRUE;
	for(int row = 0; row <  GetColumnCount(); row++)
	{						
		if((!bPaintFixed)&&(GetItemState(row,nColumn) & GVIS_FIXED))
			continue;
		bRet = SetItemFgColour(row,nColumn,cr);		
		if(!bRet)
			break;
	}
	return bRet;
...
BOOL CGridCtrl::SetRowHeight(int nRow, int height,/*Novodranov ->*/BOOL bResetScroll)
{
...
BOOL CGridCtrl::SetColumnWidth(int nCol, int width,/*Novodranov ->*/BOOL bResetScroll)
{
...
void CGridCtrl::EnsureVisible(int nRow, int nCol)
{
    ...
    */
    ...
    {
		if (pFocusWnd && ::IsWindow(pFocusWnd->GetSafeHwnd()))//<-Novodranov
			pFocusWnd->SetFocus(); 
        return;
    }
	...
	if(rectWindow.IsRectEmpty())//<-Novodranov
	{
		return;
	}
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    else if (m_MouseMode == MOUSE_SIZING_COL)
    {
		...
		//Novodranov
		SendMessageToParent(0, 0, GVN_COLUMNRESIZE);
    }
...
void CGridCtrl::PrintHeader(CDC *pDC, CPrintInfo *pInfo)
{
	...
	//Novodranov
    CString strRight;
	if(!m_strRightHeader.IsEmpty())
		strRight = m_strRightHeader;
	else
		strRight.LoadString(AFX_IDS_APP_TITLE);

    // print parent window title in the centre (Gert Rijs)
    CString strCenter;
	if(!m_strCenterHeader.IsEmpty())
		strCenter = m_strCenterHeader;
	else
	{
		CWnd *pParentWnd = GetParent();
		while (pParentWnd)
	...
	//Novodranov
	if(m_lPrintFontWeight != -1)
		lf.lfWeight = m_lPrintFontWeight;
	else
		lf.lfWeight = FW_BOLD;

	if(m_lPrintFontWidth !=-1)
		lf.lfWidth = m_lPrintFontWidth;
	if(m_lPrintFontHeight != -1)
		lf.lfHeight = m_lPrintFontHeight;
...
void CGridCtrl::PrintFooter(CDC *pDC, CPrintInfo *pInfo)
{
    ...
    CString strLeft;
	//CString	m_strLeftFooter;
	//CString	m_strRightFooter;
	if(m_strLeftFooter.IsEmpty())
		strLeft.Format(_T("Страница %d из %d"), pInfo->m_nCurPage, pInfo->GetMaxPage() );
	else
		strLeft = m_strLeftFooter;
	...
	if(m_strRightFooter.IsEmpty())
		//strRight = time.Format(_T("%c"));
		strRight.Format(_T("%02d.%02d.%02d %02d:%02d:%02d"), time.GetDay(), time.GetMonth(), time.GetYear(), time.GetHour(), time.GetMinute(), time.GetSecond());
	else
		strRight = m_strRightFooter;
...
{
    CString strCurrentText = GetItemText(nRow, nCol);
    //if (strCurrentText != str) //Novodranov for GridCellCombo
    {
...
void CGridCtrl::OnFind(void)
{
	...
	m_pFindDialog->Create(TRUE, _T(""), NULL, FR_DOWN|FR_FINDNEXT, this);
}
int nLastRow = 0;
int nLastCol = 0;
CString FindWhatLast;
LRESULT CGridCtrl::OnFindDialogMessage(WPARAM wParam, LPARAM lParam)
{
    ASSERT(m_pFindDialog != NULL);
    ...
    if(m_pFindDialog->FindNext())
    {
        ...
        CString FindWhat = m_pFindDialog->GetFindString();		
        BOOL bMatchCase = m_pFindDialog->MatchCase();
        BOOL bMatchWholeWord = m_pFindDialog->MatchWholeWord();
        BOOL bSearchDown = m_pFindDialog->SearchDown();
		
		//with given name do search

		CCellID cell = this->GetFocusCell();	

		if(!bMatchCase)
		{
			CharLower(FindWhatLast.GetBuffer());
			CharLower(FindWhat.GetBuffer());	
		}
		if((FindWhatLast != FindWhat)||(nLastRow != cell.row)||(nLastCol != cell.col))
		{
			FindWhatLast = FindWhat;
			nLastRow = cell.row;
			nLastCol = cell.col;
		}
		...
		if(bSearchDown)
		{
			for(int i = cell.row; i < this->GetRowCount(); i++)
			{
				for(int j = 0; j < this->GetColumnCount(); j++)
				{
					if((bMatchCase)&&(bMatchWholeWord))
					{
						if((this->GetItemText(i,j) == FindWhat)
							&&((i > nLastRow )||(j > nLastCol)))
						{
							this->SetFocusCell(i,j);
							this->EnsureVisible(i,j);
							nLastRow = i;
							nLastCol = j;
							FindWhatLast = FindWhat;
							return 0;
					...
					if((!bMatchCase)&&(bMatchWholeWord))
					{
						CString tmp = this->GetItemText(i,j);
						
						CharLower(tmp.GetBuffer());
						CharLower(FindWhat.GetBuffer());						
						if((tmp == FindWhat)&&((i > nLastRow )||(j > nLastCol)))
						{
							this->SetFocusCell(i,j);
							this->EnsureVisible(i,j);
							nLastRow = i;
					...
					{
						CString tmp = this->GetItemText(i,j);
						if((tmp.Find(FindWhat) != -1)&&((i > nLastRow )||(j > nLastCol)))
						{
							this->SetFocusCell(i,j);
							this->EnsureVisible(i,j);
							nLastRow = i;
		...
		nLastRow = cell.row;
		nLastCol = cell.col;
		AfxMessageBox(_T("Ничего не найдено!"),MB_ICONINFORMATION|MB_OK);
...
void CGridCtrl::CopyToClipboard()
{
	CString str;
	...
	LPTSTR extext;
    DWORD lng = str.GetLength();

	if (::OpenClipboard(NULL))
	{
		::EmptyClipboard();   
	    
		HGLOBAL gl=::GlobalAlloc(GMEM_MOVEABLE, (lng+1)*sizeof(TCHAR));
	 
		extext=(LPTSTR)::GlobalLock(gl);
		memcpy(extext, str.GetBuffer(0), (lng)*sizeof(TCHAR));
		extext[lng]=(TCHAR)0;
